...
EOF
    ...
    case "$OS" in
        ...
        linux)
            ...
            ;;
        *)
            # Default to Windows or Linux amd64
            MONACO_BINARY="monaco-linux-amd64"
            ;;
    esac
...
run_monaco_v2_with_retry() {
    ...
    for ((i=1; i<=MAX_RETRIES; i++)); do
        ...
        run_monaco_v2 "$MONACO_PROJECT" "$DASHBOARD_OWNER"
        RESULT=$?
...
case "$SETUP_TYPE" in
    "k8")
        echo "Setup type = k8"
        download_monaco_v2
        # Monaco v2 has better dependency resolution, but SLO timing issues may still occur
        # Using retry logic instead of running twice blindly
        run_monaco_v2_with_retry k8
        ;;
    "services-vm")
        echo "Setup type = services-vm"
        download_monaco_v2
        run_monaco_v2_with_retry services-vm
        ;;
    "synthetics")
        echo "Setup type = synthetics"
        # Synthetics don't have dependencies, single run is fine
        run_monaco_v2 synthetics
        ;;
    "dashboard")
        if [ -z $DASHBOARD_OWNER_EMAIL ]; then
            echo "ABORT dashboard owner email is required argument"
            echo "syntax: ./setup-workshop-config-v2.sh dashboard name@company.com"
            exit 1
        else
            echo "Setup type = dashboard"
            run_monaco_v2 db $DASHBOARD_OWNER_EMAIL
        fi
        ;;
    *)
        ...
        # Monaco v2 has better dependency resolution, but SLO timing issues may still occur
        # Using retry logic instead of running twice blindly
        run_monaco_v2_with_retry workshop
        run_custom_dynatrace_config
        ;;
esac
